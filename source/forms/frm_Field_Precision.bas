...
On Error GoTo Err_CanopyGap_Click
  ...
  DoCmd.OpenQuery "qry_Clear_Field_Precision"
  DoCmd.SetWarnings True

'  Build SQL statement
  strSQL = "SELECT * FROM qry_Field_Precision WHERE 1 = 1 "
  ...
  ' Count records by station, parameter
   ...
   If Obsvalues.EOF Then
     MsgBox "No valid Result records found.", vbOKOnly, "Percentile Report"
     Obsvalues.Close
   ...
   MaxValue = Obsvalues!Precision
   MinValue = Obsvalues!Precision
   ValueSum = 0
   DoCmd.SetWarnings False
   DoCmd.OpenQuery "qry_Clear_StdDev"
   DoCmd.SetWarnings True
   Set SDWork = db.OpenRecordset("tbl_wrk_StdDev")
   Set WorkOutput = db.OpenRecordset("tbl_wrk_Field_Precision")
   Do Until Obsvalues.EOF
     ...
     End If
     SDWork.AddNew
     SDWork!Precision = Obsvalues!Precision
     SDWork.Update
     RecordCount = RecordCount + 1    ' Count the record
     dEndDate = Obsvalues!START_DATE  ' Set end of date range
     If Obsvalues!Precision < MinValue Then
       MinValue = Obsvalues!Precision
     ElseIf Obsvalues!Precision > MaxValue Then
       MaxValue = Obsvalues!Precision
     End If
     ValueSum = ValueSum + Obsvalues!Precision
     Obsvalues.MoveNext
   Loop
   ' Write last record
   SDWork.Close
...
Exit_CanopyGap_Click:
    ...
    MsgBox "Finished - results are in tbl_wrk_Field_Precision."
    Exit Sub
