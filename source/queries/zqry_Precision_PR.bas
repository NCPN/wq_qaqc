Operation =1
Option =0
Having ="(((tblCharacteristics.FIELD_LAB)=\"Field\") AND ((IIf(IsNumeric([Precision]),1,0"
    "))=1))"
Begin InputTables
    Name ="tblProjects"
    Name ="tblLocations"
...
Begin OutputColumns
    ...
    Expression ="Min(tblVisits.START_DATE)"
    Alias ="MaxOfSTART_DATE"
    Expression ="Max(tblVisits.START_DATE)"
    Alias ="CountOfSTART_DATE"
    Expression ="Count(tblVisits.START_DATE)"
End
Begin Joins
...
Begin Groups
    ...
    GroupLevel =0
    Expression ="IIf(IsNumeric([Precision]),1,0)"
    GroupLevel =0
End
